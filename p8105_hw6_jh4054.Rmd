...
```{r}
# Stepwise Backward Elimination Model
...
# adjusted R-square criteria Model
fit_rs = lm(bwt ~ babysex + bhead + blength + delwt + gaweeks + mrace + smoken + pnumsga, data = bw)
summary(fit_rs)

# add predictions and residuals to adjusted r-square model
bw_fit_rs = modelr::add_predictions(bw, fit_step, var = "pred_rs")
bw_fit_rs = modelr::add_residuals(bw_fit_step, fit_step, var = "resid_rs")
```
...
```{r}
plot(fit_step$residuals ~ bw$bhead)
plot(fit_step$residuals ~ bw$blength)
plot(fit_step$residuals ~ bw$delwt)
plot(fit_step$residuals ~ bw$gaweeks)
plot(fit_step$residuals ~ bw$smoken)
plot(fit_step$residuals ~ bw$ppbmi)
plot(fit_step$residuals ~ bw$parity)
plot(fit_step$residuals ~ bw$fincome)

bw_fit_step %>% 
  ggplot(aes(x = bhead, y = resid_step)) +
  geom_point()
```
...
Both the stepwise regression model and adjusted r-squared model satisfied the four principal assumptions that justify the use of a linear regression model. 

We prefer the parsimonious model, the simplest best model, thus we will select the R~2~(adj) model with 8 predictors over the stepwise regression model with 11 predictors.
