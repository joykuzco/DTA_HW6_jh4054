...
cv_bw = cv_bw %>% 
  mutate(lm_rs = map(train, ~lm(bwt ~ bhead + blength + delwt + gaweeks + mrace + ppbmi + smoken, data = .x)),
         lm_2 = map(train, ~lm(bwt ~ blength, gaweeks, data = bw)),
         lm_3 = map(train, ~lm(bwt ~ bhead * blength * babysex, data = bw))) %>% 
  mutate(rmse_rs = map2_dbl(lm_rs, test, ~rmse(model = .x, data = .y)),
...
cv_bw %>% 
  ...
  mutate(
    ...
    model = str_replace(model, "2", "2_predictors"),
    model = str_replace(model, "3", "3_predictors"),
